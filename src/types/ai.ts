...
export interface AISessionSubmitResult {
   ...
   */
  readonly sessionId: QraftSessionId;
...
export interface QueueStatus {
   ...
   */
  readonly runningSessionIds: readonly QraftSessionId[];
...
/**
 * QraftSessionId - Branded type for QraftBox internal session identifiers.
 *
 * Generated by the session manager (format: "session_{base36_timestamp}_{random}").
 * Distinct from ClaudeSessionId (assigned by Claude CLI) and QraftAiSessionId
 * (hash-based group key for prompt continuity).
 */
export type QraftSessionId = string & {
  readonly __brand: "QraftSessionId";
};
