...
describe("createClaudeSessionsStore", () => {
  ...
  describe("fetchProjects", () => {
    it("should fetch projects successfully", async () => {
      ...
      expect(fetchMock).toHaveBeenCalledWith(
        "/api/ctx/test-ctx/claude-sessions/projects",
      );
  ...
  describe("fetchSessions", () => {
    ...
    it("should fetch sessions successfully", async () => {
      ...
      expect(fetchMock).toHaveBeenCalledWith(
        expect.stringContaining("/api/ctx/test-ctx/claude-sessions/sessions?"),
      );
    ...
    it("should include filters in query params", async () => {
      ...
      const callArg = (fetchMock as unknown as ReturnType<typeof vi.fn>).mock
        .calls[0]?.[0] as string;
  ...
  describe("resumeSession", () => {
    it("should resume session successfully", async () => {
      ...
      expect(fetchMock).toHaveBeenCalledWith(
        "/api/ctx/test-ctx/claude-sessions/sessions/session-123/resume",
  ...
  describe("setInitialFilters", () => {
    it("should set filters without triggering fetch", () => {
      store.setInitialFilters({
        workingDirectoryPrefix: "/g/gits/tacogips/QraftBox",
      });

      expect(store.filters.workingDirectoryPrefix).toBe(
        "/g/gits/tacogips/QraftBox",
      );
      expect(fetchMock).not.toHaveBeenCalled();
    });

    it("should set multiple initial filters", () => {
      store.setInitialFilters({
        workingDirectoryPrefix: "/g/gits",
        source: "claude-cli",
      });
      ...
      expect(store.filters.workingDirectoryPrefix).toBe("/g/gits");
      expect(store.filters.source).toBe("claude-cli");
      expect(fetchMock).not.toHaveBeenCalled();
    });
  });
});
